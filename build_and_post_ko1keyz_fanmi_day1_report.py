...
blocks.append(p([
    ...
    f'CD予約特典・タワレコ限定応募券など、デビューシングル関連のキャンペーンの詳細は<a href="{DEBUT_EVENTS_URL}" target="_blank" rel="noopener">KO1KEYZデビューシングル特典まとめ！SHOWCASE招待・ファンミ抽選会・タワレコ限定応募券の記事</a>で、事前EC販売やグッズラインナップは<a href="{GOODS_EC_URL}" target="_blank" rel="noopener">KO1KEYZ 1STファンミ グッズ・事前EC販売まとめの記事</a>であわせて紹介しています。',
]))
...
blocks.append(p([
    ...
    "一方でスタンド席の入場では本人確認がなかったという報告も見られ、座席種別によって確認の厳しさに差があった可能性があります。",
    "手荷物検査もあわせて実施されていたとの投稿があるため、まだ参戦していない公演がある人は、顔写真付きの身分証明書を忘れずに用意しておくと安心です。",
]))
...
blocks.append(p([
    ...
    f'グッズのラインナップや事前EC販売については<a href="{GOODS_EC_URL}" target="_blank" rel="noopener">KO1KEYZ 1STファンミ グッズ・事前EC販売まとめの記事</a>、FCブース限定のトレカ・くじについては<a href="{FANCLUB_BOOTH_URL}" target="_blank" rel="noopener">KO1KEYZファンミFCブースでトレカ・くじがもらえる?の記事</a>で詳しく紹介しています。',
]))
...
blocks.append(p([
    ...
    "1件の投稿では「古家さんと恋文企画」や「とわすけのミニコーナー」といったトーク企画がセットリストの合間に挟まれていたとの報告もありましたが、こちらは他の投稿での裏付けが取れていないため、正式な構成としてではなく現地の一つの声として留めておきます。",
]))

blocks.append(h2("アンコールの撮影OKタイム・トロッコはどうだった？"))
blocks.append(mini_box('<p style="margin:0;">アンコール1曲目「Run Again」のみ撮影OK。トロッコはアリーナ席の外周通路を1周する演出でした。</p>'))
blocks.append(p([
    "アンコールで披露されたRun Againのみが撮影OKの楽曲で、現地のファンからは「近くに来てくれてありがとう」「撮るのに慣れてなさすぎて」といった興奮気味の投稿が続々と上がりました。",
