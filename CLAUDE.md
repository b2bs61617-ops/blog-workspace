...
| トラジャ(chomoand-4.blog)アイキャッチ生成(`tools/eyecatch_torahja.py`) | **chomoand-4.blogの記事を作ったら、確認なしで自動でこれを使ってアイキャッチを作る**(2026-09-19〜、既存44記事は適用済み)。`--title`+`--split "1行目\|2行目\|3行目"`の3行デザイン(2行目=一番強調したい内容、Travis Japan/トラジャは省略、背景=タイトルのメンバーカラー/グループは紫)。手順は[blog-upload](.claude/skills/blog-upload/SKILL.md)STEP4、仕様は[docs/eyecatch-style.md](docs/eyecatch-style.md) |
...
| 検索順位チェック(`tools/rank_check.py`) | 指定キーワード×記事の実際の検索順位をその場で確認(2026-09-22追加)。`python tools/rank_check.py "キーワード" ドメイン`。**Yahoo!検索のみ対応**(Googleはヘッドレスアクセスを検知しブロックするため非対応)。Googleの順位はSearch Console API併用か目視確認で補う。詳細は[docs/tools.md](docs/tools.md) |
