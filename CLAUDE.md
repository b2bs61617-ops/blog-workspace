...
- **Canva MCP**(`https://mcp.canva.com/mcp`): chomoand-1.comのアイキャッチをCanvaの本物のテンプレ・フォントで作るための公式リモートMCP(2026-07-15追加、`.mcp.json`に登録済み)。**PCごとに`/mcp`からの認証が必要**。接続状況・運用フロー・プランごとの制限は[docs/canva-mcp.md](docs/canva-mcp.md)参照。
