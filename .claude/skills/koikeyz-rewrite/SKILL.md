...
- **毎朝のLINE通知**(2026-07-05追加、2026-07-05に「毎日必ず通知」に変更): 監視結果に関わらず**毎朝必ず1通LINE通知する**(`tools/line_notify.py`の`notify()`を呼ぶ)。内容は3パターン:
  - 新着投稿が無い → 「本日は新着情報なしワン」
  - 新着投稿はあるがGemini要約プロンプトの末尾判定(`判定: あり`/`判定: なし`)で「記事化に値する情報なし」 → その旨だけ通知
