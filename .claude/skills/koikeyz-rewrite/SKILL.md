...
- **ノイズ除外**: トレカ交換・好き顔診断コピペ・アフィリエイト広告(`#PR`等)・同行募集などの定型ノイズはレポート生成前にスクリプト側(正規表現)で除外し、同一告知文の重複投稿も1件にまとめる(マツが読むトークン量削減のため、2026-07-05追加)。**既知の課題**: `#PR`は大文字前提の正規表現になっており小文字`#pr`はすり抜ける、ファンミの譲渡・交換・同行募集も微妙な言い回し(絵文字混じり・改行区切りなど)だと`NOISE_PATTERNS`をすり抜けることがある(2026-07-05、生データ目視確認で判明)。気づいたときに`x_monitor.py`の`NOISE_PATTERNS`を強化するとよい。
