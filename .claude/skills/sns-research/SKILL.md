...
- X: 投稿テキスト・日時・画像(原寸 `name=orig`)を取得。**2026-08-02〜各ツイートの詳細ページを個別に開いてリプライ(コメント)も最大10件取得**(その分ページ遷移が増え収集速度は低下)
- Instagram: グリッドを左上から順にクリック→モーダルから画像・キャプション・日時・**コメント欄(2026-08-02〜)**を取得
