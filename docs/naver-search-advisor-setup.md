...
**現在の状態(2026-09-06)**: セットアップ完了・稼働中。キー検証ファイル `https://chomoand-1.com/ff9c46dd9d99fc9060510f013f108d69.txt` は設置済みで200を返す(2026-09-06に生存確認)。IndexNow送信もHTTP 200で受理されることを確認済み。

**注意(2026-09-06):** この時期、`.env` から `NAVER_INDEXNOW_KEY` の行が消えていた(ローカル・Googleドライブ両方)。キー自体は上記の通り生きているので、`.env` に `NAVER_INDEXNOW_KEY=ff9c46dd9d99fc9060510f013f108d69` を入れ直すだけで復旧する(Googleドライブ側 `G:\マイドライブ\ブログ関係\.env` を編集。ローカルはSessionStartフックで上書きされる)。IndexNowキーは秘密情報ではないので、消えたらこの値をそのまま再設定してよい。
