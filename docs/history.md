...
### 再発・恒久対応を「執筆ルール」から「変換処理の機械的除去」に変更(2026-08-16)

上記の対応から4日後、ユーザーから「直ってない、他のPCでKO1KEYZの記事を書いた時、線出てた」と再度報告があった。確認したところ、2026-08-16に別セッション・別PCで書かれた新規下書き3件(ID11339/11341/11353)全てで同じ`<hr>`(リード文直後の区切り線)が再発していた。

- **根本原因**: 2026-08-12の対応はdocs/rules.md・blog-upload/SKILL.mdへの「リード文直後に`---`を書かない」という執筆時の注意書きに留まっていた。複数PC・複数セッションが独立して動く運用では、執筆担当のセッションがこの注意書きを毎回思い出す保証がなく、実際に忘れられて再発した。
- **恒久対応(変更)**: [.claude/skills/blog-upload/SKILL.mdのSTEP1.5](../.claude/skills/blog-upload/SKILL.md)に機械的ルールを追加(commit c06bd5a)。**記事内で最初に登場する`<hr>`がリード文(記事内最初の段落)の直後にある場合、`wp:separator`に変換せず`<hr>`ごと削除する**処理に変更。執筆時に`---`を書いてしまっても、STEP1.5の変換時点で自動的に取り除かれるため、執筆側の記憶に依存しなくなった。2つ目以降の`<hr>`(本文中盤の意図的な区切り)は従来通り`wp:separator`に変換する。
- **既存下書きへの遡及対応(2周目)**: chomoand-1.comの全下書き(この時点で34件)に対して同じ検出・除去処理を再実行。新規に4件(ID: 11290, 11312, 11341, 11353)を修正。ID11339は再確認時点で既に区切り線が無く(他セッションが並行して直した可能性)対応不要だった。2026-08-12の11件と合わせて、確認できた範囲では**chomoand-1.comの下書き全件がクリーンな状態**になった。

**Why**: 「ドキュメントに書いた注意書きだけでは複数セッション運用で徹底されない」という教訓が重要。同種の「執筆時に気をつけて」系のルールも、機械的に強制できる工程(変換処理・投稿前チェックなど)があるなら、そちらに倒すべきという判断材料になる。

**How to apply**: 今後、同じような「見た目の不具合が繰り返し再発する」相談が来たら、まず対応が「執筆時の注意書き」だけに留まっていないか確認する。可能なら[blog-upload](../.claude/skills/blog-upload/SKILL.md)のSTEP1.5のような機械的な変換・検証ステップに落とし込み、LLMの記憶頼みにしない。
