...
3. **3行で作る**。**2行目=タイトルで一番強調したい内容(答え・オチ)**で、特大+濃色マーカー。**画面いっぱいに使う**(2026-09-19追加指示): 行の高さは1.0で行間を40px確保、1・3行目は基準112px(旧の約2倍)、2行目は基準230px+**縦に最大1.7倍引き伸ばして**残りの高さを使い切る(短い語は横にも最大1.3倍)。長い行は先に**横だけ詰めて縦長の文字**にし(1・3行目は0.62倍、2行目は0.7倍まで)、それでも収まらなければ文字を小さくする。1・3行目を長く(15文字超)すると詰まって小さくなるので、`--split`は1・3行目をなるべく短くする。
4. **背景色はタイトルで自動決定**(`--color-key`省略=auto): メンバー名(フルネーム・名字・被らない名前)が**1人だけ**出てくればそのメンバーカラーの背景、**グループ全体の話題・メンバー不在・複数人なら紫**(ステージライト)。
5. **背景画像は記事ごとにPollinations.aiでAI生成する**(2026-09-22〜、トモキ指示)。メンバーカラー(または紫)を反映した抽象的なステージ照明/水彩グラデーションのプロンプトで、**人物・顔は一切描かせない**(実写・肖像権リスクを避けるため。chomoand.comの背景生成と同じ考え方)。既定で有効、`--seed`で固定可能。生成失敗時(タイムアウト等)は従来のCSSグラデーション背景に自動フォールバックする。`--no-ai-bg`でCSS背景に固定できる(検証用)。
   - Pollinations.aiは無料利用だと`nologo=true`を付けても右下に自社ロゴが焼き込まれる(2026-09-22時点で確認、referrerパラメータでも消せない)。表示キャンバスより横220px・縦70px大きく生成し、CSSで右下だけ画面外にクロップしてロゴを隠している(`BG_OVERSCAN_W`/`BG_OVERSCAN_H`)。
...
### エレガントスタイル(`--style elegant`、2026-09-22〜、トモキ指示)

同じ3行デザインのルール(タイトル軸・グループ名省略・2行目強調・メンバーカラー自動判定・AI背景)を保ったまま、見た目だけを明朝体(Noto Serif JP)+水彩フラワーボーダーの背景に差し替えるスタイル。極太丸ゴシック+ビビッドな`stage`/`light`とは別の、上品なテイストを出したい記事用。

```bash
python tools/eyecatch_torahja.py \
  --title "【Travis Japan】トラジャがコストコ食材で作った全9品は？レシピも！" \
  --lines "7人が本気で挑んだ|全9品|担当は？レシピも公開" \
  --style elegant \
  --out images/xxx_eyecatch.png
```

- 通常の`stage`/`light`とのデザイン差分: フォントは明朝体(Noto Serif JP)、2行目の強調は色ブロックではなく細いメンバーカラーの下線、2行目の縦引き伸ばしはしない(等倍)、1・3行目は基準117px(2行目は基準132px)。
- **背景は固定アセット+hue-rotateで色替え**(2026-09-22、トモキ指示で確定): 最初はPollinations.aiで毎回AI生成していたが、構図が安定しない(中央に大きな花が生成される)・「frame」という単語にAIが反応して偽の額縁を描き二重フレーム化する、といった問題が解決しなかったため撤回。代わりにトモキが提供した紫の水彩フラワー画像(左右の縁に花、中央はクリーム無地)を`assets/eyecatch_torahja_floral_bg.png`に固定で置き、CSSの`hue-rotate`(`FLORAL_HUE_ROTATE`、色ごとの回転角は元画像の色相264°からの差分で算出)でメンバーカラー/グループ紫に変える。AI生成ではないため`--ai-bg`/`--seed`はelegantでは無視される。
- **`--lines "1行目|2行目|3行目"`**: `--split`と違い、`--title`との一致チェックをしない自由文言モード。マツが記事本文を読んで「一番キャッチーで目を引きそうな内容」を3行に組み立てる場合に使う(`--split`と同時指定は不可)。タイトルをそのまま使いたいときは従来通り`--split`を使う。
- 背景色(メンバーカラー/紫)の判定ルールは他スタイルと同じ: タイトルが特定の1人の話なら個人色、**グループ全体・複数人・メンバー不在の話題なら紫**。
...
- **韓国語版(chomoand-1.comのKR下書き)は`--lang kr`を付けて、`--top`/`--main`/`--bottom`に韓国語訳を渡す**(2026-08-13〜、ユーザー指示)。`--lang kr`にするとフォントがBlack Han Sans(ハングル対応の極太フォント、OFLライセンス、`assets/fonts/BlackHanSans-Regular.ttf`)に切り替わる。未指定(既定`--lang ja`)のままKR記事に使うと文字が日本語のまま(または英字メンバー名以外読めない)になるので注意。

```bash
python tools/eyecatch_koikeyz.py \
  --top "팬미팅 장소는?" \
  --main "KO1KEYZ" \
  --bottom "접근성과 회장 규모를 조사!" \
  --lang kr \
  --out images/ko1keyz_xxx_eyecatch_kr.png
```

適用範囲: 2026-08-13時点でユーザーが指示したのは既存の未公開KR下書き記事の直しも含む。今後KR記事のアイキャッチを作る/直すときは必ず`--lang kr`を使うこと。
...
### Travis Japanメンバーカラー適用(2026-08-09〜、ユーザー指示)

Travis Japanの記事は、汎用テンプレのランダム配色ではなく**メンバーカラーに揃える**。`tools/eyecatch_chomoand0.py`の`--color-key`で指定する(公式の強い色味そのままではなく、パステル調に薄めた配色を`TRAVIS_JAPAN_COLORS`に定義済み)。

- **メンバー個人の記事**: 該当メンバー名を`--color-key`に指定する。
  | メンバー | 公式カラー | color-key |
  |---|---|---|
  | 宮近海斗 | 赤 | `宮近海斗` |
  | 中村海人 | 緑 | `中村海人` |
  | 七五三掛龍也 | ピンク | `七五三掛龍也` |
  | 川島如恵留 | 白 | `川島如恵留`(白はそのままだと背景に溶けるためシルバーグレー寄りで代用) |
  | 吉澤閑也 | 黄 | `吉澤閑也` |
  | 松田元太 | 青 | `松田元太` |
  | 松倉海斗 | オレンジ | `松倉海斗` |
- **グループ全体を扱う記事**(複数メンバー横断・グループ単位の話題)は`--color-key "group"`(グループカラーの紫)を使う。
- 複数メンバーが同程度に主役の記事(例: 2人のコーデ比較)で1人に絞れない場合は`--color-key`を省略し、従来通りランダム(または`--seed`で固定)でよい。

```bash
python tools/eyecatch_chomoand0.py \
  --top "Travis Japan" \
  --main "松倉海斗" \
  --bottom "Chrome Heartsの指輪を調査" \
  --color-key "松倉海斗" \
  --out images/matsukura_kaito_chromehearts_eyecatch.png
```

Why: グループのメンバーカラーは公式に定着したブランドで、記事のアイキャッチを見ただけで「誰の記事か」が一目でわかる利点がある。ただし公式カラーそのまま(原色)は「現職みたいで強すぎる」とのことで、既存の汎用テンプレのパステルトーンに合わせて薄めた。

**`--main`には人物名だけを短く入れる(2026-08-20判明)**: 記事タイトルの疑問文(「◯◯のディズニー私服ネックレスは?」等)をまるごと`--main`に入れると、自動折り返しで「は?」だけが3行目に浮くなど不格好なレイアウトになる。松田元太のディズニー私服記事で実際にこれが起きて「アイキャッチなんか見づらいよ」と指摘された。`--main`は人物名のみ(例: `--main "松田元太"`)、記事トピックの疑問文・回答は`--bottom`を2回指定して2行に分ける(例: `--bottom "ディズニー私服ネックレスは?" --bottom "ティファニーと判明"`)のが正解。「今までのように名前が一番目立つ書き方がいいな」という指示で確定した。
