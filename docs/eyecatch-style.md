...
- **背景はPollinations.aiで水彩フラワーボーダーを生成**(2026-09-22〜、トモキが参考画像で指示): 四辺・四隅に花と葉が密集し、中央は文字用に大きく白抜きになる構図。花の色はメンバーカラー(またはグループなら紫)に連動する(`MEMBER_COLOR_WORDS`を流用)。生成失敗時は淡いグラデーション+ぼかしblobのCSSフォールバックになる。
