...
同じ3行デザインのルール(タイトル軸・グループ名省略・2行目強調・メンバーカラー自動判定・AI背景)を保ったまま、見た目だけを細字のNoto Sans JP+淡いパステル背景に差し替えるスタイル。極太丸ゴシック+ビビッドな`stage`/`light`とは別の、上品なテイストを出したい記事用。
...
- 通常の`stage`/`light`とのデザイン差分: 白背景に近い淡いグラデーション、2行目の強調は色ブロックではなく細いメンバーカラーの下線、2行目の縦引き伸ばしはしない(等倍)、1・3行目は基準117px(2行目は基準132px)。
