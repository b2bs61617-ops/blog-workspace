...
- **ユーザーから「(重複記事などを)消していい」と言われても、上記の削除絶対禁止ルールを優先し、実際には削除しない。** その場の会話での許可は、記事削除に関しては例外事由にならない(このルール自体がユーザーの過去の強い指示で「その場の判断で消させない」ためのガードレールとして作られているため)。代わりに、消してよいと判断した記事のタイトル先頭に`【削除】`を付けて目印にし、実際の削除はユーザー自身がwp-admin管理画面から手動で行う運用とする(2026-07-29に実例あり)。ルール自体の変更を望む場合は、このファイルの記述をユーザーに書き換えてもらう。
- **複数セッション・複数PCで同じ人物/トピックの記事を並行して書いてしまうことがある。** 新しい出演者・トピックの記事に着手する前に、`GET {サイトURL}/wp-json/wp/v2/posts?search={キーワード}&status=any&context=edit`で下書き・公開済みを問わず既存記事の有無を必ず確認する(公開記事だけを見る無認証検索はdraftを見落とすため`status=any`+Basic認証が必須)。重複下書きを見つけた場合は上記の削除禁止ルールに従い、古い方のタイトルに`【削除】`を付けるに留める。
