...
**コメント欄収集(2026-08-02〜)**: 投稿本文・画像に加えてコメント欄(投稿者名+本文)も収集し、`posts.txt`とAI分析プロンプトに含める。Instagramは投稿を開いたモーダル内に既にコメントが表示されているため追加コストなしで取得。Xはタイムライン/検索結果の一覧にリプライが出ないため、ツイートごとに詳細ページを`context.new_page()`で個別に開いて取得する(`fetch_x_replies`、最大10件/ツイート)。この分ページ遷移が増えるため収集速度が落ち、bot検知のリスクも一覧スクロールのみの場合より上がる(トレードオフ承知の上でユーザーが有効化を選択)。
