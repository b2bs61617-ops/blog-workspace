...
X投稿に貼られた手描きの間取り図(部屋割り予想イラストなど)を元にオリジナル図版を作るとき、チャットに表示される縮小サムネイルの見た目だけで文字・配置を読み取ると誤読しやすい。

- **元画像ファイルをPillowで対象領域だけcrop→2倍程度にresize(`Image.LANCZOS`)して保存し、その拡大画像をReadツールで確認してから清書きする**。KO1KEYZの部屋割り予想記事で、縮小表示のまま読み取って「バス・トイレ」「部屋③④が左右並び」と誤読し、実際は「バス・クローゼット」「部屋③④は右側に縦並び(2×2グリッドではない)」という全く違う構造だったことが2回続けて判明した。
- 手描き文字の色がメンバーの公式カラー(グループ紹介記事などで確認できる一覧)と一致するかを照合すると、読み取り精度の裏取りになる。KO1KEYZの例では全員分の私物カラーが公式メンバーカラーと一致しており、読み取りの確信度を大きく上げられた。
- Why: ユーザーから「部屋の位置とか違くない?」「よく見比べてよ」と複数回指摘され、最終的に元画像を高解像度で切り出して初めて正しい構造(部屋③④が2×2グリッドではなく右側縦並び、③の並び順もYOSHIKI→ISSA→RYUJIが正)だと判明した。
- How to apply: 手描き/手書きのSNS画像を元にオリジナル図版(見取り図・相関図など)を作る依頼が来たら、着手前に必ず元画像ファイルを`Image.crop()`→`resize()`で該当箇所だけ拡大したものをReadツールで見てから作図する。ユーザーから「位置が違う」等の指摘を受けた場合は特に、縮小表示のままの再確認で済ませず、毎回この高解像度チェックをやり直す。
