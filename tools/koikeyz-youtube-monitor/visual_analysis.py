...
文字起こしでは「会話内容」しか分からず、視聴者(KO1LY)が知りたい服装・アクセサリー・
訪問場所は映像を見ないと分からないため、フレーム画像+Vision対応AIで補う。
tools/youtube-talent-monitor/visual_analysis.py(chomoand-0向け)と同じ設計(2026-07-30導入)。

ffmpeg不要の設計: yt-dlpで低解像度(480p以下)の直リンクを取得し、opencv-pythonの
VideoCapture.set(CAP_PROP_POS_MSEC)でシークしてキャプチャする(動画をディスクに
フルダウンロードしない)。Gemini呼び出しはkoikeyz-monitor(x_monitor.py)・
youtube-talent-monitorと同じgoogle-genai SDK・gemini-flash-latestを流用(このスクリプト
単体で完結させ、日次の自動監視から直接呼ばれる想定)。

抽出したフレームは frames/{video_id}/ に保存して残す。Gitには含めない(著作権のある
動画フレーム画像のためリポジトリに含めるべきではない。.gitignore参照)。

実行(単体テスト用):
  python tools/koikeyz-youtube-monitor/visual_analysis.py <video_idまたはURL> [<動画タイトル>]
"""
import json
import sys
...
GEMINI_MODEL = "gemini-flash-latest"  # 固定バージョン名だとGoogle側の廃止で突然404になることがあるため、常に最新のflashを指すエイリアスを使う

VISION_PROMPT_TEMPLATE = """以下はYouTube動画「{title}」から一定間隔で抜き出した{count}枚の画像です。
この動画に出演しているKO1KEYZ(コイキーズ)メンバーまたはPRODUCE 101 JAPAN新世界出演者について、ブログ記事の参考情報として次の観点を日本語で箇条書きにしてください。
