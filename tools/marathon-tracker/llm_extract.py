...
- (@tv_pin) は追跡班アカウントが投げた地図ピン(緯度,経度)を機械的に読んだもの。(@share_map) と同格で信頼度が高い。文中の（緯度,経度）を map_query にそのまま入れてよい。(@share_map) と食い違うときは、より新しい(投稿時刻が近い)方を採用する。
...
- ただし (@heading) が「約0.2km以上 移動」を示しているときは、走者は前進しているので原則 update=true にし、entries を1件出す。前回と同じ町名でも「◯◯付近を通過」という通過点として1行足してよい(current_location もその通過点に更新する)。GPS上ほぼ動いていない(<0.2km)のに新しい位置情報も無い場合だけ update=false。
