...
【出どころの信頼度】
- (@share_map) は追跡者(@YSB_DANCHO)のGPS位置共有から取った現在地。**いちばん信頼度が高い**。文中の（緯度,経度）の数値があれば、その entry の map_query に「35.51240,139.54352」のように緯度,経度をそのまま入れる(正確なピンになる)。「位置情報の更新」が数分前なら現在地として断定してよい。
