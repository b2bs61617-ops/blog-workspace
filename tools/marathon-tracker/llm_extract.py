...
【出どころの信頼度】
- (@share_map) は追跡者(@YSB_DANCHO)のリアルタイム位置共有から取った現在地(GPS実座標を逆引きした住所)。**いちばん信頼度が高い**。ここに出る市区町村・町丁目を現在地として断定してよい(「位置情報の更新」が数分前なら尚更)。文中に（緯度,経度）があれば map_query にその数値をそのまま入れる。無ければ住所を入れる。
